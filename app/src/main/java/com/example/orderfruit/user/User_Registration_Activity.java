        ...
        register_btn.setOnClickListener(new View.OnClickListener() {
            ...
            private void checkPassword() {
                if (!getPassword1.equals(getPassword2)) {
                    ...
                    Boolean ab = commonDB.registrationDAO().checkUser(getPhone);

                    if (ab == true) {
                        Toast.makeText(User_Registration_Activity.this, "User Already exist", Toast.LENGTH_SHORT).show();
                    } else {
                        passwordvalid();
                        //Toast.makeText(User_Registration_Activity.this, "User Not Already exist", Toast.LENGTH_SHORT).show();
                    }
                }
            }
            ...
            private void passwordvalid() {
                if (getPassword1.length() < 8 && getPassword2.length() < 8) {
                    ...
                    //  boolean insert = commonDB.registrationDAO().insertImage(img);


                    commonDB.registrationDAO().insert(new RegistrationModel(getName, getPhone, getPassword2, 0, "", "", "", "", img));
